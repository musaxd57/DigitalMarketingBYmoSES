...
// In-memory seed status per tenant
const seedStatus = new Map();

demoRouter.get('/status', authenticate, async (req, res) => {
  const tenantId = req.user.tenantId;
  const status = seedStatus.get(tenantId);
  if (status) return res.json(status);
  // Check if demo data exists in DB
  try {
    ...
    if (parseInt(r.rows[0].count) > 0) {
      return res.json({ status: 'completed', seeded: true });
    }
...
demoRouter.post('/seed', authenticate, async (req, res) => {
  ...
  try {
    ...
    if (parseInt(existing.rows[0].count) > 0) {
      return res.json({ message: 'Demo verisi zaten mevcut', seeded: false, status: 'completed' });
    }
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }

  // Respond immediately — seed in background
  seedStatus.set(tenantId, { status: 'processing' });
  res.status(202).json({ message: 'Demo veri yükleniyor...', status: 'processing' });
  ...
  setImmediate(async () => {
    ...
    try {
      ...
      // Build bulk INSERT for all 540 snapshots at once
      ...
      for (const camp of camps) {
        ...
        for (let d = 89; d >= 0; d--) {
          ...
          snapValues.push(`($${pi},$${pi+1},$${pi+2},$${pi+3},$${pi+4},'daily',$${pi+5},$${pi+6},$${pi+7},$${pi+8},$${pi+9},$${pi+10},$${pi+11},$${pi+12},$${pi+13},$${pi+14},$${pi+15})`);
          snapParams.push(
      ...
      await client.query(
        `INSERT INTO analytics_snapshots
         ...
         VALUES ${snapValues.join(',')}`,
        snapParams
      );

      await client.query('COMMIT');
      const snapCount = snapValues.length;
      logger.info(`[Demo] Seeded tenant ${tenantId}: ${camps.length} campaigns, ${snapCount} snapshots`);
      seedStatus.set(tenantId, { status: 'completed', seeded: true, campaigns: camps.length, snapshots: snapCount });
      setTimeout(() => seedStatus.delete(tenantId), 300000);
    } catch (err) {
      await client.query('ROLLBACK').catch(() => {});
      logger.error('[Demo] Seed error:', err.message);
      seedStatus.set(tenantId, { status: 'failed', error: err.message });
      setTimeout(() => seedStatus.delete(tenantId), 60000);
    } finally {
      client.release();
    }
  });
